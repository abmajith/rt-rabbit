...
module rf_packet_framer #(
    parameter [15:0] EOF_MARKER = 16'h0AC5,   // Compile-time footer pattern
    parameter [15:0] MAX_PAYLOAD = 16'd512    // memory safety ceiling
)(
    ...
    output reg         framer_hunting_window_o, // High when waiting for a new packet frame

    // Hardware Storage Engine / Wishbone DMA Allocator Drivers
    output reg  [15:0] dma_data_o,          // payload word
    ...
    output reg  [15:0] out_packet_idx,      // current sequence index
    output reg  [15:0] out_data_type,       // current data frame classification payload
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            // State Machine moves when aligner serves a valid word
            if (word_valid_i || (sof_detected_i && current_state == STATE_IDLE)) begin
                case (current_state)
                    STATE_IDLE: begin
                        if (sof_detected_i) begin
                            framer_hunting_window_o <= 1'b0; // Lock to a new data frame
                    ...
                    STATE_TYPE: begin
                        out_data_type <= aligned_word_i; // frame data packet type
